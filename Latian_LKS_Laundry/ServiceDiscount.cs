...
{
    public partial class ServiceDiscount : Form
    {
        ...
        void DataGrid()
        {
            string query = "select servicediscount.id_discount, service.id_service, servicediscount.name_discount as 'Name', service.name_service as 'Service', servicediscount.discount as 'Discount', servicediscount.expired_discount as 'Expired Date' from servicediscount join service on servicediscount.id_service = service.id_service";
        ...
        private void ServiceDiscount_Load(object sender, EventArgs e)
        {
            Color Color2 = System.Drawing.ColorTranslator.FromHtml("#495664");
            Serv.BackColor = Color2;
        ...
        private void Del_Click(object sender, EventArgs e)
        {
            if (id > 0)
            {
                ...
                if (result == DialogResult.OK)
                {
                    try
                    {
                        string delete = "delete from servicediscount where id_discount=" + id + "";
        ...
        private void SavUp_Click(object sender, EventArgs e)
        {
            if (ValidateIn())
            {
                try
                {
                    string update = "update servicediscount set id_service=" + comboBox1.SelectedValue + ", name_discount='" + textBox1.Text + "', discount=" + numericUpDown1.Value + ", expired_discount='" + dateTimePicker1.Value.Date + "' where id_discount="+ id +"";
        ...
        private void SavIn_Click(object sender, EventArgs e)
        {
            if (ValidateIn())
            {
                try
                {
                    string insert = "insert into servicediscount values ("+ comboBox1.SelectedValue +",'"+ textBox1.Text +"', "+ numericUpDown1.Value +", '"+ dateTimePicker1.Value.Date +"')";
