...
{
    public partial class InputPackageTransactionForm : Form
    {
        SqlConnection connection = new SqlConnection(Utils.conn);
        SqlCommand cmd;
        SqlDataReader reader;
        public Point mouseMove;
        public InputPackageTransactionForm()
        {
            InitializeComponent();
        }
        ...
        void DataGrid()
        {
            ...
            dataGridView1.Columns[2].Name = "Package Price";
        ...
        private void InputPackageTransactionForm_Load(object sender, EventArgs e)
        {
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Transaction Field Must Be Filled", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                try
                {
                    connection.Open();
                    cmd = new SqlCommand("select * from package where id_package = '" + comboBox1.SelectedValue + "'", connection);
                    reader = cmd.ExecuteReader();
                    reader.Read();

                    int id = Convert.ToInt32(reader["id_package"]);
                    string name = Convert.ToString(reader["name_package"]);
                    int price = Convert.ToInt32(reader["price_package"]);
                    int quantity = Convert.ToInt32(numericUpDown1.Value);
                    string duration = Convert.ToString(reader["duration_package"]);
                    int subtotal = Convert.ToInt32(price * quantity);
                    ...
                    string[] add = { Convert.ToString(id), name, Convert.ToString(price), Convert.ToString(quantity), duration, discount.ToString(), Convert.ToString(subtotal) };
        ...
        {
            comboBox2.Text = "";
            string discount = "select * from packagediscount where id_package = " + comboBox1.SelectedValue + " and expired_discount >= CONVERT(DATE, GETDATE())";
        ...
        private void InputPackageTransactionForm_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString("LKS Laundry", new Font("Arial", 30, FontStyle.Regular), Brushes.Black, new Point(300, 10));

            e.Graphics.DrawString("Transaction Date :", font, Brushes.Black, new Point(10, 150));
            e.Graphics.DrawString(date.ToString(), font, Brushes.Black, new Point(500, 150));

            e.Graphics.DrawString("Cashier : " + Model.Name.ToString(), font, Brushes.Black, new Point(10, 200));
            e.Graphics.DrawString("Customer : " + textBox1.Text, font, Brushes.Black, new Point(10, 250));

            int offsetY = 350;
            for(int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                e.Graphics.DrawString(dataGridView1.Rows[i].Cells[1].Value.ToString(), font, Brushes.Black ,new Point(10, offsetY));
                e.Graphics.DrawString(dataGridView1.Rows[i].Cells[3].Value.ToString(), font, Brushes.Black, new Point(270, offsetY));
                e.Graphics.DrawString(dataGridView1.Rows[i].Cells[2].Value.ToString(), font, Brushes.Black, new Point(570, offsetY));
                offsetY += 50;
            }
            e.Graphics.DrawString("_____________", font, Brushes.Black, new Point(570, offsetY));
            offsetY += 50;
            e.Graphics.DrawString("Total : ", font, Brushes.Black, new Point(10, offsetY));
            e.Graphics.DrawString("Rp. " + labelPrice.Text, font, Brushes.Black, new Point(570, offsetY));
        }
    }
}
